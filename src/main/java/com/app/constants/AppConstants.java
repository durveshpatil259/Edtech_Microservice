package com.app.constants;

public class AppConstants {

    public static final String EMPTY_STR = "";  //constant variables should be in capital
    public static final String PLAN_SAVE_SUCCESS = "planSaveSuccess";
    public static final String PLAN_SAVE_FAIL = "planSaveFail";
    public static final String PLAN_UPDATE_SUCCESS = "planUpdateSucess";
    public static final String PLAN_UPDATE_FAIL = "planUpdateFail";
    public static final String PLAN_DELETE_SUCCESS = "planDeleteSucess";
    public static final String PLAN_DELETE_FAIL = "planDeleteFail";
    public static final String PLAN_STATUS_CHANGE_SUCCESS = "planStatusChangeSuccess";
    public static final String PLAN_STATUS_CHANGE_FAIL = "planStatusChangeFail";
}
